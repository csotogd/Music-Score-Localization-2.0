...
def evaluate(raw_ref, fs_ref, raw_recording, fs_record, recording_labels, length_snippet_secs = 3):
    """
    ...
    for time_recording, true_label  in recording_labels:
        ...
        recording_interval = get_song_interval(raw_recording, time_recording, fs_record,length_sec= length_snippet_secs)
        constellation_record = sp_pipeline(recording_interval, fs_record, denoise=True)

        #use shazam algorithm to localize snippet:
        prediction =localization_pipeline(constellation_ref, constellation_record, time_ahead= 10, sample_freq= fs_record) #MUST be the recording Fs and not the reference one
...
def evaluate_localization_one_point(true_label, prediction, interval_side_perfect_length = 1, interval_side_relevant_lenght = 2):
    """
    ...
    if (prediction >= true_label- interval_side_perfect_length) and (prediction<= true_label+ interval_side_perfect_length):
        return 1

    elif (prediction <= true_label - interval_side_perfect_length) and (prediction>= true_label - interval_side_perfect_length - interval_side_relevant_lenght):
        distance_from_0_score = prediction - (true_label - interval_side_perfect_length - interval_side_relevant_lenght)
        score = distance_from_0_score / interval_side_relevant_lenght
        return score

    elif (prediction >= true_label + interval_side_perfect_length) and ( prediction <= true_label + interval_side_perfect_length + interval_side_relevant_lenght):
        distance_from_0_score =  (true_label + interval_side_perfect_length + interval_side_relevant_lenght) - prediction
        score = distance_from_0_score / interval_side_relevant_lenght
        return score
...
if __name__ == '__main__':
    ...
    Fs_ref, ref_song = read("../data/bach_prelude_c_major/Bach_prelude_original_1channel.wav")
    ...
    paths = [(path1_rec, path1_labels), (path2_rec, path2_labels), (path3_rec,  path3_labels)]
    ...
    for path_rec, path_labels in paths:
        ...
        for length_snippet in length_snippets_in_secs:
            ...
            score = evaluate(raw_ref=ref_song, fs_ref=Fs_ref, raw_recording=record_song, fs_record=Fs_record,
                               recording_labels=labelled_data, length_snippet_secs=length_snippet)
            scores.append(score)
    ...
    for i in range(len(names)):
        for j in range(len(length_snippets_in_secs)):
            print("score for ", names[i], " and snippet of ", length_snippets_in_secs[j]," seconds ---->", scores[i*len(length_snippets_in_secs) + j ])
        print()
